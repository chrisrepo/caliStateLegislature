<?php
 ...
 $badHtml = 0;
 set_time_limit(0);
 $sql = "INSERT INTO billVotes (billNumber, yesVotes, noVotes, yesMembers, noMembers, voteDate, location, result, nvrVotes, nvrMembers, motion) VALUES ";
 foreach($billIDs as $id) {
    ...
    for ($i = 0; $i < $totalVotes; $i++){
        ...
        $strAdd = '("'.$vote->measureNumber.'",'.$vote->yesVotes.','.$vote->noVotes.',"'.$vote->yesMembers;
        $strAdd = $strAdd.'","'.$vote->noMembers.'","'.$vote->voteDate.'","'.$vote->location.'","'.$vote->result;
        $strAdd = $strAdd.'",'.$vote->nvrVotes.',"'.$vote->nvrMembers.'","'.$vote->motion.'"), ';
